...
const Badge = ({ badge, earned }) => (
  <div className={`relative flex flex-col items-center p-5 border rounded-2xl transition-all duration-300 ${
    earned 
      ? 'glass-panel shadow-md transform hover:-translate-y-1 hover:border-amber-400/40 dark:hover:border-amber-500/30' 
      : 'border-2 border-dashed border-amber-250/20 dark:border-slate-800/50 bg-white/10 dark:bg-slate-950/10 filter grayscale opacity-45'
  }`}>
    <div className="mb-2.5">{badge.icon}</div>
    <h3 className="text-md font-bold text-gray-800 dark:text-gray-200">{badge.name}</h3>
    <p className="text-xs text-gray-550 dark:text-gray-400 text-center mt-1">{badge.description}</p>
    {earned && (
...
const Achievements = ({ completedCourses = [], allCourses = [] }) => {
  ...
  return (
    <div className="glass-panel p-6 shadow-inner rounded-2xl">
      <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-6">Achievements & Badges</h2>
