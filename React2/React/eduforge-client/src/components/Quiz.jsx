...
const Quiz = ({ quizData, courseId, onComplete }) => {
  ...
  const handleSubmitQuiz = async () => {
    setLoading(true);
    try {
      const response = await api.post(`/courses/${courseId}/quizzes/${quizData._id}/submit`, {
  ...
  const getOptionClass = (option) => {
    ...
    if (isReviewMode) {
      const correctAnswerId = results.correctAnswers[currentQuestion._id];
        ...
        <div className="space-y-3">
          {currentQuestion.options.map((option) => (
            ...
            >
              ...
              {isReviewMode && (
                <>
                  {option._id === results.correctAnswers[currentQuestion._id] && <Check className="w-6 h-6 text-green-500" />}
                  {selectedAnswers[currentQuestion._id] === option._id && option._id !== results.correctAnswers[currentQuestion._id] && <X className="w-6 h-6 text-red-500" />}
                </>
              )}
            </button>
          ))}
