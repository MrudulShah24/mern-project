...
exports.getRecommendations = async (req, res) => {
  try {
    ...
    const [
      dynamicPersonalized,
      dynamicCollaborative,
      dynamicTrending,
      dynamicNextSteps
    ] = await Promise.all([
      populateEnrolledCount(contentBasedCourses),
      populateEnrolledCount(collaborativeCourses),
      populateEnrolledCount(trendingCourses),
      populateEnrolledCount(nextLevelCourses)
    ]);

    res.json({
      personalized: dynamicPersonalized,
      collaborative: dynamicCollaborative,
